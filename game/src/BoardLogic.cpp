...
{
    int oldGridX = node->GetVar("GridX").GetI32();
    int oldGridY = node->GetVar("GridY").GetI32();
...
{
    int gridX = node->GetVar("GridX").GetI32();
    int gridY = node->GetVar("GridY").GetI32();
...
int BoardLogic::GetLineLength(int startX, int startY, const IntVector2& dir)
{
    ...
    int firstColor = firstUnit->GetVar("ColorIndex").GetI32();
    int count = 1;
        ...
        gridX += dir.x_, gridY += dir.y_)
    {
        ...
        int color = unit->GetVar("ColorIndex").GetI32();
        if (color != firstColor)
            break;
...
void BoardLogic::MarkToRemove(Vector<bool>& output,
...
{
    return String("w") + String(width_) + "h" + String(height_) +
        "c" + String(numColors_) + "p" + String(initialPopulation_) +
        "l" + String(lineLength_) + "d" + String(diagonal_);
}
